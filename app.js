...
app.get('/upsertDB',
  async (req,res,next) => {
    ...
    for (game of games){
      const {title,steamUrl,publisher,genres,status}=game;
      await Course.findOneAndUpdate({title,steamUrl,publisher,genres,status},game,{upsert:true})
    }
...
app.post('/games/byPublisher', 
    async(req,res,next)=> {
      ...
      var regex = new RegExp(publisher, "gi")
      const games = await Course.find({publisher:regex}).sort({title:1})
...
app.post('/games/byName',
    async(req, res, next) => {
        ...
        var regex_name = new RegExp(title, "gi")
        const games = await Course.find({title:regex_name}).sort({title:1})
        res.locals.games = games
        res.render('courselist')
    }
)
...
app.post('/games/byGenre',
    async(req, res, next) => {
        ...
        var regex_name = new RegExp(genre, "gi")
        const games = await Course.find({genres:regex_name}).sort({title:1})
        res.locals.games = games
        res.render('courselist')
    }
)
